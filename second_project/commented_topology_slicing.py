...
class TrafficSlicing(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        if dpid in self.mac_to_port:
            if dst in self.host_mac_address:
                self.logger.info("####################################################################")
                self.logger.info("dpid = %s src = %s   dst = %s", dpid, src, dst)
            self.logger.info("first if")
            if dst in self.mac_to_port[dpid]:
                ...
                self._send_package(msg, datapath, in_port, actions)
                self.logger.info("udpUP ---- s%s in_port=%s out_port=%s", dpid, in_port, out_port)
                ...
                self.add_flow(datapath, 2, match, actions)
                self._send_package(msg, datapath, in_port, actions)
                self.logger.info("udpDW ---- s%s in_port=%s out_port=%s", dpid, in_port, out_port)
            ...
            self._send_package(msg, datapath, in_port, actions)
            self.logger.info("dpid = %s src = %s   dst = %s", dpid, src, dst)
            self.logger.info("flooding ---- s%s in_port=%s out_port=%s", dpid, in_port, out_port)

        else:
            self.logger.info("secondo else tutto")
